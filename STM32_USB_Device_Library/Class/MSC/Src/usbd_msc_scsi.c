...
extern uint8_t isRead;
	...
	extern uint8_t gray_image[NUM_OF_PIXELS];
...
static int8_t SCSI_ImageInfoRead (USBD_HandleTypeDef  *pdev, uint8_t lun)
{
  ...
  uint32_t len;
  uint8_t *info_buffer;
	uint8_t image_info[5];
	
	image_info[0] = IMAGE_TYPE;
	image_info[1] = NC & 0xFF;
	image_info[2] = (NC >> 8) & 0xFF;
	image_info[3] = NR & 0xFF;
	image_info[4] = (NR >> 8) & 0xFF;
